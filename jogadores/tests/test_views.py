...
#         
# class ViewListarPedidosFeriasTestCase(TestCase):
#     """Testes para a view de listar pedidos de férias"""
#     @classmethod
#     def setUpTestData(cls):
#         super(ViewListarPedidosFeriasTestCase, cls).setUpTestData()
#         criar_jogadores_teste(['sena', 'teets',])
#         cls.jogador_1 = Jogador.objects.get(nick='sena')
#         cls.jogador_2 = Jogador.objects.get(nick='teets')
#         
#         # Pedido pendente
#         cls.pedido_pendente = PedidoFerias.objects.create(jogador=cls.jogador_1, data_inicio=timezone.localtime() - datetime.timedelta(days=10),
#             data_fim=timezone.localtime() + datetime.timedelta(days=10))
#             
#         # Pedido validado
#         cls.pedido_validado = PedidoFerias.objects.create(jogador=cls.jogador_2, data_inicio=timezone.localtime() - datetime.timedelta(days=10),
#             data_fim=timezone.localtime() + datetime.timedelta(days=10))
#         
#     def test_acesso_deslogado(self):
#         """Testa acesso a tela de validar pedido de férias sem logar"""
#         response = self.client.get(reverse('jogadores:listar_pedidos_ferias'))
#         self.assertEqual(response.status_code, 200)
#         
#         # Verificar 2 pedidos e possíveis ações
#         self.assertEqual(len(response.context['pedidos']), 2)
#         self.assertIn(self.pedido_pendente, response.context['pedidos'])
#         self.assertIn(self.pedido_validado, response.context['pedidos'])
#         self.assertNotContains(response, reverse('jogadores:validar_pedido_ferias', {'pedido_id': self.pedido_pendente.id})
#         self.assertNotContains(response, reverse('jogadores:validar_pedido_ferias', {'pedido_id': self.pedido_validado.id})
#         
#     def test_acesso_logado(self):
#         """Testa acesso a tela de validar pedido de férias logado"""
#         self.client.login(username=self.jogador_1.user.username, password=SENHA_TESTE)
...
#         self.assertIn(self.pedido_pendente, response.context['pedidos'])
#         self.assertIn(self.pedido_validado, response.context['pedidos'])
#         self.assertContains(response, reverse('jogadores:validar_pedido_ferias', {'pedido_id': self.pedido_pendente.id})
#         self.assertNotContains(response, reverse('jogadores:validar_pedido_ferias', {'pedido_id': self.pedido_validado.id})
#         
# class ViewEncerrarPedidoFeriasTestCase(TestCase):
#     """Testes para a view de encerrar pedido de férias"""
#     @classmethod
#     def setUpTestData(cls):
#         super(ViewEncerrarPedidoFeriasTestCase, cls).setUpTestData()
#         criar_jogadores_teste(['sena', 'teets',])
#         cls.jogador_1 = Jogador.objects.get(nick='sena')
#         cls.jogador_2 = Jogador.objects.get(nick='teets')
#         
...
#     def test_acesso_deslogado(self):
#         """Testa acesso a tela de validar pedido de férias sem logar"""
#         response = self.client.get(reverse('jogadores:encerrar_pedido_ferias'))
#         url_esperada = settings.LOGIN_URL + '?next=' + reverse('jogadores:encerrar_pedido_ferias')
#         self.assertRedirects(response, url_esperada)
#         self.assertEqual(response.status_code, 302)
#         
#     def test_acesso_logado(self):
...
#         self.assertEqual(response.status_code, 200)
#         
#     def test_encerrar_pedido(self):
#         """Testa validar um pedido com sucesso"""
#         self.client.login(username=self.jogador_2.user.username, password=SENHA_TESTE)
#         response = self.client.post(reverse('jogadores:encerrar_pedido_ferias'), {'salvar': 1})
#         self.assertEqual(response.status_code, 302)
#         
#         messages = list(response.context['messages'])
#         self.assertEqual(len(messages), 1)
#         self.assertEqual(str(messages[0]), RegistroFerias.MENSAGEM_SUCESSO_ENCERRAMENTO_FERIAS)
#         
#         self.assertRedirects(response, reverse('jogadores:detalhar_jogador', kwargs={'username': self.jogador_1.user.username}))
